...
\paragraph{Note:}
\small{}%
Filters and corresponding options allso may be assembled inside a special meta
...
On commandline than \verb#add<metafilter># is specified instead of the filters and options assembling it.
\normalsize  
...
In any other case or in case of questing the configuration of a specific option +e allways has to be followed by ``<\,{}filtername\,{}>'', ``<\,{}regex\,{}>'' or a literal ``all''.
...
\paragraph{Note:}
The \textbf{tex} filter mode is allso available via \textbf{latex} alias name.
