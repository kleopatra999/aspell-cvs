...
add or removes a path where aspell should look for filter ojects beeing loadable during runtime. If the object library of a filter can not be found, Aspell will refuse toload the filter.
\item [add|rem-option-path]\textit{(list)}add or removes a path where the files containing the description of a filter (recognized options, version of Aspell the filter is desigend for, ...) is located. If this information can not be found or is corrupt aspell will refuse to load the entire filter.
...
These options may be prefixed by the keyword \texttt{filter-} in order to explicitly indicate that they are options recognized by a filter and not by aspell itselve.
\begin{description}
\item[email] This filter hides quoting characters and email preamgle and other parts of an email which need not to be spellchecked.
\begin{description}
...
the quote char
\end{description}
\item[sgml] This filter converts an sgml sourcefile into a format which eases spellchecking of sgml texts by aspell.
\begin{description}
\item [sgml-check]\textit{(list)} sgml attributes to always check.
\item [sgml-extension]\textit{(list)} sgml file extensions.
\end{description}
\item[tex|latex]This filter hides all latex commands and corresponding paramters not beeing readable text in latex output from Aspell.
\begin{description}
\item [tex-command]\textit{(list)} \TeX{} commands
\item [tex-check-comments]\textit{(boolean)} check \TeX{} comments
\end{description}
\item[context]
This filter can be used to spellcheck source codes, html sources and othet texts which consist of different contexts.
These contexts must be separated by pairs of unique delimiters.
The different contexts may not be dependent uppon each other except for initial context which is assumed if not any other context applies.
\begin{description}
\item[context-visible-first]\textit{(boolean)}
Switches the context which should be visible to aspell.
Per default the initial context is assumed to be invisible as one would expect when spellchecking source files of programms where relevant parts are contained in string constants and comments but not in the remaining code.
...
If multible pairs are specified by one add|rem-context-delimiters call the different pairs have to be separated by a literal comma.
...
The parameter +e and its argument allways must be specified immediatly after config command and before specifying name of option in order to be recognized by Aspell.
   
\section{Printing Help creen}
...
In order to quest available filter(s) and/or the corresponding options call Aspells help comand whith the name of the entire filter or a regular expression.
...
The visible ones will be spellchecked and the invisible ones will be ignored.
The contexts are distinguised by the fact that the visible/invisible ones are delimited by specific and unique delimiter characters or character sequences.
Whether the delimited contexts schould be visible or invisible only  stated by the value of the \textbf{{[}dont-{]}context-visible-first} option and not by the delimiters.
...
To indicate that a context is allways closed by end of line use \verb#\0# sequence as closing delimiter.
