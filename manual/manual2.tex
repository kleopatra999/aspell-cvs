...
\paragraph{Note:}
\small{}%
Filters and corresponding options also may be assembled inside a special meta
filter file named ``<metafilter>.flt''.
A filter has to be loaded via adding a \verb#add-filter <filtername># line to the meta filter file before it's options may be specified.

On command line than \verb#add<metafilter># is specified instead of the filters and options assembling it.
\normalsize  
...
If this option is specified than Aspell with either use this dictionary
or die.
...
These options modify the behavior of Aspell filter interface in general (see section
\ref{filter} for more information):

\begin{description}
\item [add|rem-filter]\textit{(list)} add or removes a filter
\item [add|rem-filter-path]\textit{(list)}
add or removes a path where Aspell should look for filter objects being loadable during runtime. If the object library of a filter can not be found, Aspell will refuse to load the filter.
This path is also searched when trying to load a meta filter.
\item [add|rem-option-path]\textit{(list)}add or removes a path where the files containing the description of a filter (recognized options, version of Aspell the filter is designed for, ...) is located. If this information can not be found or is corrupt Aspell will refuse to load the entire filter.
...
The two {}``machine unsigned'' encodings are intended to be used
by other programs using the Aspell library and it is unlikely the
Aspell utility will ever support these encodings.
\end{description}
These Options belong to filters packaged along with Aspell standard distribution.
These options may be prefixed by the keyword \texttt{filter-} in order to explicitly indicate that they are options recognized by a filter and not by Aspell itself.
\begin{description}
\item[email] This filter hides quoting characters and email preamble and other parts of an email which need not to be spell checked.
\begin{description}
...
the quote char
\end{description}
\item[sgml] This filter converts an sgml source file into a format which eases spell checking of sgml texts by Aspell.
\begin{description}
\item [sgml-check]\textit{(list)} sgml attributes to always check.
\item [sgml-extension]\textit{(list)} sgml file extensions.
\end{description}
\item[tex|latex]This filter hides all latex commands and corresponding parameters not being readable text in latex output from Aspell.
\begin{description}
\item [tex-command]\textit{(list)} \TeX{} commands
\item [tex-check-comments]\textit{(boolean)} check \TeX{} comments
\item [tex-multi-byte]\textit{(list)} \TeX{}  multi byte letter en|decoding 
\end{description}
\item[context]
This filter can be used to spell check source codes, html sources and other texts which consist of different contexts.
These contexts must be separated by pairs of unique delimiters.
The different contexts may not be dependent upon each other except for initial context which is assumed if not any other context applies.
\begin{description}
\item[context-visible-first]\textit{(boolean)}
Switches the context which should be visible to Aspell.
Per default the initial context is assumed to be invisible as one would expect when spell checking source files of programs where relevant parts are contained in string constants and comments but not in the remaining code.
If set to true the initial context is visible while the delimited ones are hidden. 
\item[add|rem-context-delimiters]\textit{(list)}
Add or remove pairs of delimiters.
This allows to specify the characters\footnote{\label{fn:delimiting_chars}this may also be sequences of chars} which should be used to switch contexts and and therefore have to be escaped by \\ if they should appear literally.
The two delimiting chars\textsuperscript{\ref{fn:delimiting_chars}} belonging to one pair have to be separated by a space character.
If multiple pairs are specified by one add|rem-context-delimiters call the different pairs have to be separated by a literal comma.
Per default the delimiters are set to \texttt{C/C++} comment and string constant delimiters.
If the end of line delimits a context than this has to be indicated by the literal ``\verb#\0#'' string.
\end{description}
\end{description}
...
Misc. other options that don't fall under any other category
...
The default configuration of a particular filter may be quested by passing the name of the filter or a corresponding regular expression to the config command.
This is done by passing +e <\,{}expr\,{}> to dump config command of Aspell.
If +e is the only argument to dump config Aspell assumes ``+e all'' which lists the configuration of all installed and reachable filters.
In any other case or in case of questing the configuration of a specific option +e always has to be followed by ``<\,{}filtername\,{}>'', ``<\,{}regex\,{}>'' or a literal ``all''.
The parameter +e and its argument always must be specified immediately after config command and before specifying name of option in order to be recognized by Aspell.
   
\section{Printing Help screen}

To print the help screen of Aspell simply invoke Aspell with help command or by -? parameter.
This will list all basic options and commands of Aspell but not the available filters and their options.

In order to quest available filter(s) and/or the corresponding options call Aspell's help command with the name of the entire filter or a regular expression.
If corresponding filter(s) are installed and reachable via filter-path and option-path setting Aspell will in addition to it's default help screen print a short description of the filter and its options.

If Aspell help command is called with keyword \verb#all#, all installed and available filters and the corresponding options are listed. 
...
use the \textbf{mode} option\footnote{this may be removed in future as right now it is possible to define meta filters}. You may chose from \textbf{none}, \textbf{url},
...
filters are \textbf{url}, \textbf{email}, \textbf{sgml}, \textbf{tex}, \textbf{latex}(alias for \textbf{tex}), \textbf{context} 
...
controls whether or not Aspell will skip over \TeX{} comments. The
...
\paragraph{Note:}
The \TeX filter mode is also available via \textbf{latex} alias name.

The \TeX filter mode also contains a decoding and a encoding filter for \textbf{babel} character codes like the German Umlauts:
\selectlanguage{german}
\begin{itemize}
\item[\textbf{\"a}] -> \textbf{\textbackslash{}\dq{}a} -> \textbf{\dq{}a}
\item[\textbf{\"o}] -> \textbf{\textbackslash{}\dq{}o} -> \textbf{\dq{}o}
\item[\textbf{\"u}] -> \textbf{\textbackslash{}\dq{}u} -> \textbf{\dq{}u}
\item[\textbf{\"s}] -> \textbf{\textbackslash{}\dq{}s} -> \textbf{\dq{}s}
\end{itemize}
\selectlanguage{english}
...
\end{quote}
Changes list of multi character coded \TeX (babel) characters recognized by Aspell. 
In case of German umlauts mentioned above this would mean that Aspell would decode from their multi character representation to their proper single char representation.
Given the German word \texttt{St\"arke(strength)} which within \TeX/\LaTeX document has to be written as \selectlanguage{german}\texttt{St\dq{}arke} or as \texttt{St\textbackslash{}\dq{}a}\selectlanguage{english} would split it into the two words \texttt{St} and \texttt{arke} if it does not know anything about the multi char encoding \selectlanguage{german}\textbf{\dq{}a} or \textbf{\textbackslash{}\dq{}a} of \textbf{"a}\selectlanguage{english}.
On the other hand if it knows about it than Aspell will recognize the word properly and will not try to make any strange suggestion.
...
The \textbf{context} filter allows Aspell to distinguish between visible and invisible contexts.
The visible ones will be spell checked and the invisible ones will be ignored.
The contexts are distinguished by the fact that the visible/invisible ones are delimited by specific and unique delimiter characters or character sequences.
Whether the delimited contexts should be visible or invisible only  stated by the value of the \textbf{{[}dont-{]}context-visible-first} option and not by the delimiters.

The context delimiters are specified as pairs of delimiters via the \textbf{add|rem-context-delimiters} option.
The delimiters enclosing a specific context are specified as a space separated pair.
If more than one delimiter pair is specified by one call of \textbf{add|rem-context-delimiters} they have to be combined to a comma separated list.
To indicate that a context is always closed by end of line use \verb#\0# sequence as closing delimiter.
...
Further Aspell pipe command recoginzes the following parameters:
\begin{description}
\item[noauto] do not come up with sugestions automatically;
\item[word] stop spelling after first mispelled word if found;
\end{description}

If moauto was specified on commandline than Aspell only prints the misspelled words but does not come up with any sugestions.
In order to still retrieve sugestions without restarting Aspell a leading '?' will make aspell print them for the entire line.

In word mode a leading '§' will make Aspell seeking all mispelled words on al line instead of stopping after the first misspelling.

To summarize these: 

\begin{description}
\item[?] print sugestions for entire line in noauto mode
\item[§] seek all misspellings on the entire line
\end{description}
