...
\section*{Changes from 0.50.2 to 0.50.3 (Nov ??, 2002)}

\begin{itemize}
\item Hopefully fixed the ispell alignment error problem when Aspell is
used with ispell.el.
\item {[}Fixed a problem with NFS mounted home directories{]}
\item Compiled libaspell-common directory into libaspell for now to avoid
forcing applications to relink with ever new Aspell version due to
the use of the libtool '-release' flag.
\end{itemize}
