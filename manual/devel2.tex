...
\end{table}
The \texttt{ENDOPTION} line may be omitted as it is assumed implicitly if a line containing \texttt{OPTION} or \texttt{ENDFILE} keyword occurs or the file ends.
...
A more convenient way to build a new filter is provided by Aspell's programmers interface for filter.
...
If source files for one or more filters use the interface provided by loadable-filter-API.hpp than the filter description file described in section \ref{sec:filter_descript} can be generated automatically.
This is done by passing all source files to mk-filter-opt.pl perl-script script via
\begin{verbatim}
