...
A more convenient\footnote{recommended, if filter is added to Aspell standard distribution} way is to use the developer Interface provided by Aspell distribution.
...
The following line hints that the filter description file was generated by mk-flt-opt.pl and that it might be part of the \texttt{Aspell} standard distribution.
\begin{verbatim}
STATIC <filtertype>
\end{verbatim}
...
If done manually one has to know option retrieving mechanism of Aspell by heart.

\subsection{Compiling and Linking}
See a good book on Unix programming on how to turn the filter source into a loadable object.

\subsection{Programmers Interface\textsuperscript{\ref{fn:recommended_statndard_aspell}}\label{sec:programmers_interface}}
...
For details upon the entire macros see interfaces/filter/loadable-filter-API.hpp\,{}.
...
If source files for one or more filters use the interface provided by loadable-filter-API.hpp than the filter description file described in section \ref{sec:filter_descript} has to be generated by passing all source files\footnote{filter class definition, before member implementation} to mk-filter-opt.pl perl-script script via
\begin{verbatim}
perl mk-filter-opt.pl [<option>] <sourcefile> [ <sourcefile> ... ]
\end{verbatim}
call.
If Aspell already has been installed on the entire system mk-flt-opt.pl may be used instead via
\begin{verbatim}
mk-flt-opt.pl [<option>] <sourcefile> [<sourcefile> ...]
\end{verbatim}
call.

If \textbf{--test} option is specified for <\,{}option\,{}> the filter and option hierarchy of all filters found inside the given source file(s) is printed and developer is asked whether this is correct or not.
The filter description file(s) is(are) in case of \textbf{--test} option only generated if and only if filter hierarchy is accepted by answering yes.
If \textbf{--test} option is not specified the filter description file(s) is(are) always generated.
...
For loadable filter use \verb#$top_srcdir/runtime_modules/filter#.
For static filter use \verb#$top_srcdir/modules/filter#.
\item
Modify the entire \verb#Makefile.am# files.
The lines which have to be modified are enclosed by 
\begin{verbatim}
########MODIFY HERE BEGIN########
